    ...
    public function getFakerType( string $attributeName, object $attributeData ) : string
    {
        ...
        if( property_exists( $attributeData, 'type' ) )
        {
            if( str_contains( $attributeName, 'slug' ) && in_array( $attributeData->type, [ 'string', 'text' ] ) )
                return "\$this->faker{$unique}->slug( 5 )";
            if( str_contains( $attributeName, 'image' ) && in_array( $attributeData->type, [ 'string', 'text' ] ) )
                return "\$this->faker{$unique}->imageUrl( 640, 480, 'people' )";
            ...
            if( $attributeData->type == 'string' && in_array( $attributeName, [ 'name', 'fullname', 'full_name' ] ) )
                return "\$this->faker{$unique}->firstName() . ' ' . \$this->faker{$unique}->lastName()";
            ...
            if( $attributeData->type == 'string' && in_array( $attributeName, [ 'lastname', 'last_name' ] ) )
                return "\$this->faker{$unique}->lastName()";
            if( $attributeData->type == 'string' && in_array( $attributeName, [ 'title' ] ) )
                return "\$this->faker{$unique}->sentence( 6 )";
            ...
            if( $attributeData->type == 'string' && in_array( $attributeName, [ 'address', 'fulladdress' ] ) )
                return "\$this->faker{$unique}->address()";
            if( $attributeData->type == 'string' && in_array( $attributeName, [ 'postalcode', 'address_code' ] ) )
                return "\$this->faker{$unique}->postcode()";
            if( $attributeData->type == 'string' && in_array( $attributeName, [ 'code' ] ) )
                return "\$this->faker{$unique}->bothify('???-#####')";
            if( $attributeData->type == 'date' ) return "\$this->faker{$unique}->date()";
            if( strtolower( $attributeData->type ) == 'datetime' ) return "\$this->faker{$unique}->dateTime()";
            if( $attributeData->type == 'time' ) return "\$this->faker{$unique}->time()";
            if( $attributeData->type == 'boolean' ) return "\$this->faker{$unique}->boolean()";
            if( str_contains( $attributeName, 'id' ) && in_array( $attributeData->type, $integers ) )
            {
                return "\$this->faker{$unique}->numberBetween(1, 20)";
            }
            if( in_array( $attributeData->type, $integers ) ) return "\$this->faker{$unique}->numberBetween( 1, 1000 )";
            if( in_array( $attributeData->type, [ 'float', 'decimal' ] ) ) return "\$this->faker{$unique}->randomFloat( 2, 1, 100 )";
        }
